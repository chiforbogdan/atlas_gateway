...
AtlasTelemetry::AtlasTelemetry() : hostnameResource_(ATLAS_TELEMETRY_HOSTNAME_URI,
                                   ...
                                   kernInfoResource_(ATLAS_TELEMETRY_KERN_INFO_URI,
                                                     ATLAS_COAP_METHOD_PUT,
                                                     boost::bind(&AtlasTelemetry::kernInfoCallback, this, _1, _2, _3, _4, _5, _6, _7, _8))
{
...
AtlasCoapResponse AtlasTelemetry::kernInfoCallback(const std::string &path, const std::string &pskIdentity,
                                                   const std::string& psk, AtlasCoapMethod method,
                                                   const uint8_t* reqPayload, size_t reqPayloadLen,
                                                   uint8_t **respPayload, size_t *respPayloadLen)
{
    ...
    ATLAS_LOGGER_DEBUG("Telemetry kernel info callback executed...");

    ATLAS_LOGGER_INFO1("Process TELEMETRY KERNEL INFO command from client with DTLS PSK identity ", pskIdentity);

    AtlasDevice& device = AtlasDeviceManager::getInstance().getDevice(pskIdentity);
    if (!device.isRegistered()) {
        ATLAS_LOGGER_ERROR("Received TELEMETRY KERNEL INFO command for a device which is not registered...");
        return ATLAS_COAP_RESP_NOT_ACCEPTABLE;
    }
    ...
    if (cmd.empty()) {
        ATLAS_LOGGER_ERROR("Telemetry kernel info end-point called with empty command set");
        return ATLAS_COAP_RESP_NOT_ACCEPTABLE;
    }

    for (AtlasCommand &cmdEntry : cmd) {
        if (cmdEntry.getType() == ATLAS_CMD_TELEMETRY_KERN_INFO) {
            if (!cmdEntry.getLen()) {
                ATLAS_LOGGER_ERROR("Telemetry kernel info end-point called with invalid KERNEL INFO token length");
                return ATLAS_COAP_RESP_NOT_ACCEPTABLE;
            }
            
            kernInfo.assign((char *)cmdEntry.getVal(), cmdEntry.getLen());
        } else if (cmdEntry.getType() == ATLAS_CMD_IDENTITY) {
            if (!cmdEntry.getLen()) {
                ATLAS_LOGGER_ERROR("Telemetry kernel info end-point called with empty IDENTITY command");
                return ATLAS_COAP_RESP_NOT_ACCEPTABLE;
            }
            
            identity.assign((char *)cmdEntry.getVal(), cmdEntry.getLen());
            if (pskIdentity != identity) {
                ATLAS_LOGGER_ERROR("Telemetry kernel info end-point called with SPOOFED identity");
                return ATLAS_COAP_RESP_NOT_ACCEPTABLE;
            }
        }
    }
    ...
    if (identity == "") {
        ATLAS_LOGGER_ERROR("Telemetry kernel info failed because of invalid identity");
        return ATLAS_COAP_RESP_NOT_ACCEPTABLE;
    }
    if (kernInfo == "") {
        ATLAS_LOGGER_ERROR("Telemetry kernel info failed because of invalid kernel info");
        return ATLAS_COAP_RESP_NOT_ACCEPTABLE;
    }

    /* Set device kernel info */
    device.getTelemetryInfo().setFeature(TELEMETRY_KERN_INFO, kernInfo);
